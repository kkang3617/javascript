...
function makeNumberIcons() {
    //id=numbers 인 div 태그 안에다가 숫자 아이콘을 배치해야합니다.
    //숫자 아이콘의 개수는 객체가 가지고 있는 min, max에 따라 달라집니다.
    //숫자 아이콘은 div태그이고, 클래스이름은 icon으로 매깁니다.
    ...
    for(let i=gameData.min; i<=gameData.max; i++) {
        ...
        $icon.textContent = i;
        $frag.appendChild($icon); // frag 가상컨테이너에 icon넣음
    }

    $numbers.appendChild($frag); //가상컨테이너에 있는 icon을 $number에 추가
    return $numbers;
...
function checkAnswer($numbers, $target) {
    ...
    //begin과 end값도 변경.
    //aside태그의 애니메이션을 담당할 executeUpDownAnimation 호출.
    if(secret === answer) {
...
function processCorrect($target) { /* $target 정답아이콘 */
...
(function() {
    ...
    $numbers.addEventListener('click', function(e) {
        ...
        gameData.answer = +e.target.textContent; /* +붙임 , 문자->숫자변환 */
